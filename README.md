...
```bash
npm run setup        # checks and installs all prerequisites
                     # first run: 10–15 min
...
When the public export is built from a checkout that already has a staged Windows portable bundle, that `windows-x64-portable/` directory is carried into the external export as a prebuilt evaluation artifact, including `textforge-editor.exe` and its required `resources/` payload.
