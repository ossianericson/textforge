...
- The public export includes the desktop editor source under `editor/`, the release-layout note at `artifacts/editor/README.md`, and the staged Windows portable bundle when one exists in `artifacts/editor/<version>/windows-x64-portable/`; that UI client remains an experimental testing surface and should not be presented as a stable product.
...
Lefthook now enforces the repo pre-commit policy and staged spec validation. Run `npm run validate:spec` manually before committing spec changes when you want a full repo check outside the hook path.
...
6. It validates the generated export by running `npm install`, `npm run build`, `npm run compile`, and `npm run validate:spec` inside `dist/public-export/`.
...
**Windows WSL2:** Set `git config core.autocrlf false` before cloning. If hook execution fails with a permission error, refresh the Lefthook install and verify the shell available to Git. Prefer WSL2 if you want a Linux-like shell environment for hook behavior, but standard `npm run` commands also work from PowerShell in the current repo.
