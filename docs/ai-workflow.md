...
- The compiler is deterministic TypeScript with a test suite and golden output
  hashes. The same input always produces the same output.
...
npm test                # verifies output matches golden hash or fails if unexpected
```
...
Domain Expert describes intent
  → Architect produces prompt file
    → Coding Agent executes prompt file
      → Three closing commands run (see below)
...
## The Three Closing Commands

Every session ends with these three commands in this order.
They are non-negotiable. The Coding Agent runs them. If the session
ends without them, run them manually before merging the PR.

```bash
npm run update:changelog
```

Reads `TODO.md` and prepends an entry to `CHANGELOG.md`.
Records what was done in human-readable form.
No manual writing. No decisions.

```bash
npm run create-pr
```

Reads `TODO.md` and opens a pull request automatically.
The PR title and body come from the session summary.
No manual copy-paste.

```text
[context snapshot export step]
```

After changelog generation and PR creation, export an updated repository
context snapshot for the next session. The exact command, file names, and
storage location are repository-specific by design.

In textforge, those operational details are documented separately in an
internal companion guide that does not ship in the public export.
...
| ------------------------------ | --------------- | -------------------------------------------------------------------------------------- |
| Claude                         | Codex           | Yes — current textforge setup                                                          |
...
| Claude                         | Future agent    | Yes — prompt file format is plain markdown                                             |
...
- Golden output hashes in your test suite (detect silent regressions)
...
**5. Every session ends with three commands in order.**
`npm run update:changelog` then `npm run create-pr`.
No session is considered complete until the context export step has run and the new
context file has been provided to your AI assistant for the next session.
