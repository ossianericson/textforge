...
interface CompileParsedOptions {
  specPath: string;
...
function shouldRunAsCli(argv: string[], entryFilePath: string): boolean {
  if ((argv[1] ?? '') !== entryFilePath) {
    return false;
  }
...
function compileParsedDecisionTree({
  specPath,
  parsedSpec,
  templatePath,
  outputPath,
}: CompileParsedOptions): void {
  ...
  const parsed = sanitizeParsedSpec(parsedSpec);
  parsed.metadata = {
    ...(parsed.metadata || {}),
...
function compileDecisionTree({ specPath, templatePath, outputPath }: CompileOptions): void {
  ...
  try {
    ...
    if (error instanceof DecisionTreeCompilerError) {
      throw error;
    }
...
const isMain = shouldRunAsCli(process.argv, fileURLToPath(import.meta.url));
if (isMain) {
  main();
}
...
export { compileDecisionTree, compileParsedDecisionTree };
