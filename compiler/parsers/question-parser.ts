...
function createQuestionParser(
  ...
  function trimLegacyOptionMarkerSuffix(value: string): string {
    return value
      .replace(/\s+\*\((recommended|advanced)\)\*$/i, '')
      .replace(/\s+\((recommended|advanced)\)$/i, '')
      .trim();
  }
  ...
  function parseOption(line: string): QuestionOption | null {
    ...
    const textRaw = trimLegacyOptionMarkerSuffix(left.replace(/^\d+\.\s*/, '').trim());
    ...
    const cleanedNext = next
      .replace(/\*\((recommended|advanced)\)\*/gi, '')
