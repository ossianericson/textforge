...
function createDropdownBlockHandler(): QuestionBlockHandler {
  return {
    matches: (line) => line.startsWith('**Dropdown**'),
    parse: ({ lines, startIndex, helpers }: QuestionBlockContext) => {
      ...
      for (; i < lines.length; i += 1) {
        ...
        if (shouldEndBlock(trimmed, ['**Dropdown**'])) {
          break;
        }
...
export { createDropdownBlockHandler };
