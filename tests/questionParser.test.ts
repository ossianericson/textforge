...
test('questionParser: tolerates legacy UI option marker placement', () => {
  const lines = [
    '## Decision Tree Flow',
    '',
    '### Q1: Entry (id="q1")',
    ...
    '**Subtitle**: "One"',
    '**Options**:',
    '1. "Start guided tour (recommended)" *(Recommended)* → go to q2',
    '2. "Fallback path" *(Advanced)* → result: result-guidance',
  ];
  ...
  assert.ok(q1);
  assert.equal(q1.options.length, 2);
  assert.equal(q1.options[0].text, 'Start guided tour (recommended)');
  assert.equal(q1.options[0].next, 'q2');
  assert.equal(q1.options[0].recommended, true);
  assert.equal(q1.options[0].advanced, false);
  assert.equal(q1.options[1].text, 'Fallback path');
  assert.equal(q1.options[1].next, 'result-guidance');
  assert.equal(q1.options[1].recommended, false);
  assert.equal(q1.options[1].advanced, true);
});
