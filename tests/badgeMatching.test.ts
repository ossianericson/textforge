...
test('applyBadgeClasses: explicit badge tech tag overrides badge text fallback', () => {
  const results = {
    'result-a': {
      ...buildResult('Team approval — send this to your manager then raise an IT ticket'),
      techTags: ['IT-ticket', '.badge.standard'],
    },
  };
  const badgeCss = buildBadgeCss(['iaas', 'standard', 'advanced']);

  applyBadgeClasses(results, badgeCss);

  assert.equal(results['result-a'].badge.className, 'standard');
  assert.deepEqual(results['result-a'].techTags, ['IT-ticket']);
});
