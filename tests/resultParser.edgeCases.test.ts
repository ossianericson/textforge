...
test('resultParser: captures one copy block with verbatim content', () => {
  const lines = [
    '## Result Cards (1 Services)',
    '',
    '#### 1. Example Service (result-example)',
    ...
    '- Badge: Example (blue)',
    '',
    '**Copy Block: Email 1 — Request a Decision**',
    'To: legal@example.com',
    'Subject: Formal approval request — Tool X',
    '',
    'Body here, preserved exactly as written.',
    '',
    'Thank you,',
    '[Your name]',
    '**Best For:**',
  ...
  assert.ok(result);
  assert.ok(result.copyBlocks);
  assert.equal(result.copyBlocks.length, 1);
  assert.deepEqual(result.copyBlocks[0], {
    title: 'Email 1 — Request a Decision',
    content:
      'To: legal@example.com\n' +
      'Subject: Formal approval request — Tool X\n\n' +
      'Body here, preserved exactly as written.\n\n' +
  ...
  assert.equal(result.bestFor[0], 'Escalations');
});

test('resultParser: captures multiple copy blocks as separate entries', () => {
  const lines = [
    '## Result Cards (1 Services)',
    '',
    '#### 1. Example Service (result-example)',
  ...
  assert.ok(result);
  assert.ok(result.copyBlocks);
  assert.equal(result.copyBlocks.length, 2);
  assert.deepEqual(result.copyBlocks, [
    { title: 'Email 1', content: 'Line one' },
    { title: 'Command', content: 'npm run compile\nnpm test\n' },
  ]);
});

test('resultParser: leaves copy blocks undefined when none are present', () => {
  const lines = [
    '## Result Cards (1 Services)',
    '',
    '#### 1. Example Service (result-example)',
  ...
  assert.ok(result);
  assert.equal(result.copyBlocks, undefined);
  assert.equal(result.bestFor[0], 'General guidance');
});
