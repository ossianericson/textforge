...
Windows portable: download `textforge-editor-windows-x64-portable.zip` from the GitHub Releases page, extract it, keep the `resources/` folder next to `textforge-editor.exe`, and launch the EXE directly. No installation is required, but WebView2 still must be present on the machine. This is a testing artifact, not a supported production distribution.

Windows installer: build from source locally if you specifically want to exercise the installer flow as part of testing.

macOS: build from source locally on a macOS host if you want to test the experimental desktop client there.

When you build locally, staged release artifacts are copied into `artifacts/editor/<version>/<platform>/` at the repo root. Those staged directories are local outputs and should stay out of git.
