...
The editor is experimental. Treat it as a testing client for authoring patterns and packaging flow validation only. Do not present it as a stable, production-ready, or fully working end-user application.

## Quick start (pre-built binary)

The editor is still an experimental testing build. On Windows, prefer the portable bundle first.
When compile output must exactly match the production compiler, save `spec.md`
and use the editor compile action or the CLI interchangeably. The editor now
compiles the saved file from disk and then opens the real output folder instead
of relying on a separate preview-only path.

Windows portable: download the staged `windows-x64-portable/` artifact, keep the `resources/` folder next to `textforge-editor.exe`, and launch the EXE directly. No installation is required, but WebView2 still must be present on the machine. This is a testing artifact, not a supported production distribution.

Windows installer: download `textforge-editor_x.x.x_x64-setup.exe` from the Azure DevOps desktop pipeline artifact or the `textforge-editor-windows-x64` Azure Artifacts Universal Package and run it only if you specifically want to exercise the installer flow as part of testing.

macOS: download `textforge-editor_x.x.x_x64.dmg` from the Azure DevOps desktop pipeline artifact or the `textforge-editor-macos-x64` Azure Artifacts Universal Package, open it, and drag textforge Editor to your Applications folder only if you are testing the experimental desktop client.
...
On Windows, `npm run editor:release:win` now stages both the installer bundle and a no-install portable folder. Treat the portable folder as the default testing artifact and the installer as an additional packaging check. Neither should be described as a stable release.
...
| Compile and show output | Ctrl+P      | Cmd+P       |
| Export Mermaid diagram | Ctrl+M       | Cmd+M       |
| Undo                   | Ctrl+Z       | Cmd+Z       |

The main surface is now a WYSIWYG document editor for the canonical `spec.md` structure. Header fields, question blocks, routes, result cards, copy sections, tags, docs, and contact metadata are edited directly in the document. Saving serializes the current document state back to `spec.md`, and compile/show-output runs that saved file through the same parser and compiler pipeline used by the CLI.
