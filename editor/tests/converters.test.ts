...
import { DOC_NODE_TYPES } from '../src/lib/doc-model';
...
describe('specToDoc -> docToSpec round-trip', () => {
  ...
  it('normalizes object-shaped routing targets from editor node attrs', () => {
    const original = parseSpec(EXAMPLE);
    const doc = specToDoc(original);
    const firstQuestion = (doc.content ?? []).find((node) => node.type === DOC_NODE_TYPES.questionBlock);
    const questionBody = firstQuestion?.content?.find((node) => node.type === DOC_NODE_TYPES.questionBody);
    const optionRow = questionBody?.content?.find((node) => node.type === DOC_NODE_TYPES.optionRow);

    if (!optionRow) {
      throw new Error('Expected an option row in the example document.');
    }
    ...
    const restored = docToSpec(doc, original);
    expect(restored.questions.q1?.options?.[0]?.next).toBe('q2');
    expect(() => serialize(restored)).not.toThrow();
  });

  it('normalizes object-shaped routing targets before they enter editor node attrs', () => {
    const original = parseSpec(EXAMPLE);
    original.questions.q1 = {
      ...original.questions.q1,
      options: [
        {
          ...(original.questions.q1?.options?.[0] ?? {
            text: 'Continue',
            next: 'q2',
            recommended: false,
            advanced: false,
          }),
          next: { id: 'q2' } as unknown as string,
        },
      ],
    };
    ...
    expect(optionRow?.attrs?.target).toBe('q2');
  });
