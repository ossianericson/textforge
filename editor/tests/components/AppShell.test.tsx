...
vi.mock('@/lib/useSpecFileWatcher', () => ({
  useSpecFileWatcher: () => {},
}));

vi.mock('@/lib/useWindowCloseGuard', () => ({
  useWindowCloseGuard: () => {},
}));
...
describe('App shell', () => {
  beforeEach(() => {
    ...
    editorStoreMock.state = {
      ...
      checkEnv: vi.fn().mockResolvedValue(undefined),
  ...
  it('uses adaptive workspace without enforcing a fixed minimum window height', async () => {
  ...
  it('does not render an embedded preview dialog even when compiled output exists', async () => {
    editorStoreMock.state.showPreview = true;
    editorStoreMock.state.previewHtml = '<!DOCTYPE html><html><body><main>Preview</main></body></html>';
    editorStoreMock.state.previewPath = 'C:/workspace/preview-tree.html';

    await act(async () => {
      render(<App />);
    });

    expect(screen.queryByRole('dialog', { name: 'Compiled output preview' })).not.toBeInTheDocument();
  });
});