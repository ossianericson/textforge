...
export function CommitModal() {
  ...
  return (
    <div role="dialog" aria-modal="true" aria-labelledby="commit-modal-title" className="fixed inset-0 z-[67] flex items-center justify-center bg-black/70 p-6">
      <div className="flex max-h-[92vh] w-full max-w-2xl flex-col overflow-hidden rounded-2xl border border-slate-700 bg-slate-950 shadow-2xl">
        ...
        <div className="space-y-4 p-5">
          <label className="block text-sm text-slate-200">
            ...
            <input value={message} onChange={(event) => setMessage(event.target.value)} className="w-full rounded-xl border border-slate-700 bg-slate-900 px-4 py-3 text-sm text-slate-100" />
          </label>
          <label className="block text-sm text-slate-200">
