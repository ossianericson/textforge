...
export function RepoBrowser() {
  ...
  const [compilableOnly, setCompilableOnly] = useState(false);
  ...
  const filteredSpecs = useMemo(() => {
    ...
    if (compilableOnly) {
      list = list.filter((item) => !item.has_warnings);
    }

    return list;
  }, [compilableOnly, query, repoSpecs]);

  return (
    <ModalShell
      labelledBy="repo-browser-title"
      onClose={() => toggleRepoBrowser(false)}
      initialFocusRef={urlInputRef}
      panelClassName="mx-auto max-h-[92vh] w-full max-w-6xl overflow-y-auto rounded-2xl border border-slate-700 bg-slate-950 shadow-2xl"
            ...
            <section className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4">
              ...
              <div className="space-y-3">
                <label className="block text-sm text-slate-200">
                  ...
                  <input ref={urlInputRef} value={url} onChange={(event) => setUrl(event.target.value)} placeholder="https://github.com/org/specs.git" className="w-full rounded-xl border border-slate-700 bg-slate-950 px-4 py-3 text-sm text-slate-100" />
                </label>
                <label className="block text-sm text-slate-200">
            ...
            <div className="mb-4 flex items-center justify-between gap-4">
              ...
              <div className="flex w-full max-w-md items-center justify-end gap-3">
                <label className="flex cursor-pointer select-none items-center gap-2 text-sm text-slate-300">
                  <input
                    type="checkbox"
                    checked={compilableOnly}
                    onChange={(event) => setCompilableOnly(event.target.checked)}
                  ...
                  <div key={item.path} className="flex items-center justify-between gap-4 rounded-2xl border border-slate-800 bg-slate-950/90 px-4 py-3">
                    <div>
                      ...
                      <div className="mt-1 text-xs uppercase tracking-[0.16em] text-slate-500">
                        ...
                        {item.has_warnings ? (
                          <span
                            title="This spec has validation warnings. Validate before compiling."
                            className="ml-1.5 inline-flex items-center rounded-full bg-amber-900/50 px-1.5 py-0.5 text-xs text-amber-300"
                          >
                            ! warnings
                          </span>
                        ) : null}
