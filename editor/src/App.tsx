...
export default function App() {
  ...
  }, [loadEditorPrefs, loadRecentFiles, loadSavedRepoConnections]);
  ...
  }, [compile, openFile, saveFile, saveFileAs, toggleDashboard, toggleGenerateModal, toggleMermaidModal, toggleOnboardingTour, toggleShortcutsModal, undo]);
  ...
  return (
    <div className="flex h-dvh min-h-0 flex-col overflow-hidden bg-transparent text-slate-100">
          ...
          <>
            ...
            <div className="min-h-0 flex-1 overflow-hidden">
              {compactWorkspaceView === 'graph' && !sidebarCollapsed ? (
                ...
                <div className="h-full min-h-0">
                  <EditorErrorBoundary currentPath={currentPath}>
                    <DocumentEditor />
                  </EditorErrorBoundary>
                </div>
              )}
      ...
      {showMermaidModal ? <MermaidExportModal /> : null}
      {showGenerateModal ? <GenerateModal /> : null}
      {showDashboard ? <MyTreesDashboard /> : null}
      {showSettingsModal ? <SettingsModal /> : null}
      {showRepoBrowser ? <RepoBrowser /> : null}
      {showCommitModal ? <CommitModal /> : null}
      ...
      {showValidationPanel ? <ValidationPanel /> : null}
      {showProgressPanel ? <ProgressPanel /> : null}

      {showPreview && previewHtml ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[rgba(3,8,18,0.6)] p-6 backdrop-blur-md">
          <div className="flex h-full max-h-[92vh] w-full max-w-[1480px] flex-col overflow-hidden rounded-[30px] border border-white/10 bg-[linear-gradient(180deg,rgba(18,25,37,0.96),rgba(10,15,24,0.98))] shadow-[0_28px_90px_rgba(2,6,14,0.5)]">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/10 bg-white/[0.03] px-5 py-4 text-sm backdrop-blur-xl">
              <div>
                <div className="text-[11px] font-semibold uppercase tracking-[0.22em] text-slate-500">Preview</div>
                <span className="mt-1 block font-semibold tracking-wide text-slate-100">Compiled output</span>
              </div>
              <button onClick={togglePreview} className="rounded-2xl border border-white/10 bg-white/[0.04] px-4 py-2 text-slate-300 transition hover:border-white/15 hover:bg-white/[0.08] hover:text-white">
                Close
              </button>
            </div>
            <iframe
