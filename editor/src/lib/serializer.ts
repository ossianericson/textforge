...
import { normalizeParsedSpecRoutingTargets, normalizeRoutingTarget } from './routing-targets';
...
function formatTarget(target: string): string {
  ...
  return normalizedTarget.startsWith('result-')
    ? `result: ${normalizedTarget}`
    : `go to ${normalizedTarget}`;
}

function getQuestionIdsInDocumentOrder(questions: ParsedSpec['questions']): string[] {
...
function hasOptionMarkerInText(text: string, marker: 'recommended' | 'advanced'): boolean {
  return new RegExp(`\\(${marker}\\)`, 'i').test(text);
}

function serializeOptionSuffix(option: Question['options'][number]): string {
  const suffixes: string[] = [];
  if (option.recommended && !hasOptionMarkerInText(option.text, 'recommended')) {
    suffixes.push('(recommended)');
  }
  if (option.advanced && !hasOptionMarkerInText(option.text, 'advanced')) {
    suffixes.push('(advanced)');
  }
...
function serializeButtonsQuestion(question: Question): string[] {
  return [
    ...
    ...(question.options ?? []).map((option, index) => {
      return `${index + 1}. ${quote(option.text)} ${ARROW} ${formatTarget(option.next)}${serializeOptionSuffix(option)}`;
    }),
    '',
  ];
}
...
export function serialize(spec: ParsedSpec): string {
  spec = normalizeParsedSpecRoutingTargets(spec);
  const questionIds = getQuestionIdsInDocumentOrder(spec.questions);
